...
T5_MODEL = os.environ.get("MOCHI_T5_MODEL", "google/t5-v1_1-xxl")
    ...
    def get_model(
        ...
        *,
        ...
        model_args = dict(
            ...
            audio_cross_attn_layers=self.audio_cross_attn_layers,
            **model_kwargs,
        )
        ...
        if local_rank == 0 and load_checkpoint:
            ...
            if not strict_load:
                ...
                missing_keys = [k for k in load_result.missing_keys if "audio_" not in k and ".lora_" not in k]
                if missing_keys:
            ...
            if lora_sd:
                print("Loading adapter state dict...")
                has_audio = any("audio_" in key for key in lora_sd.keys())
                has_lora = any(".lora_" in key for key in lora_sd.keys())
                if has_audio and has_lora:
                    print("State dictionary contains both audio and LoRA parameters")
                elif has_audio:
                    print("State dictionary contains audio parameters")
                elif has_lora:
                    print("State dictionary contains LoRA parameters")
...
def sample_model(device, dit, conditioning, audio_feat=None, **args):
    ...
    def model_fn(*, z, sigma, cfg_scale):
        if cond_batched:
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                out_cond = dit(z, sigma, audio_feat=audio_feat, **cond_text)
                out_uncond = dit(z, sigma, audio_feat=audio_feat, **cond_null)
...
class MochiSingleGPUPipeline:
    ...
    def __call__(self, batch_cfg, prompt, negative_prompt, audio_feat, **kwargs):
        with torch.inference_mode():
            ...
            with move_to_device(self.dit, self.device):
                latents = sample_model(self.device, self.dit, conditioning, audio_feat, **kwargs)
            print_max_memory()
...
def cast_dit(model, dtype):
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            assert any(
                n in name for n in ["mlp", "t5", "mod_", "attn.qkv_", "attn.proj_", "final_layer", "audio_"]
...
class MochiMultiGPUPipeline:
    ...
    def __call__(self, **kwargs):
        def sample(ctx, *, batch_cfg, prompt, negative_prompt, audio_feat, **kwargs):
            with progress_bar(type="ray_tqdm", enabled=ctx.local_rank == 0), torch.inference_mode():
                ...
                latents = sample_model(ctx.device, ctx.dit, conditioning=conditioning, audio_feat=audio_feat, **kwargs)
                if ctx.local_rank == 0:
